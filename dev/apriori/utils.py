...
def map_categories(df: pd.DataFrame):
    map = {1: "Solteira", 2: "Casada", 3: "Viuva", 4: "Divorciada", 5: "União"}

    df["ESTCIVMAE"] = df["ESTCIVMAE"].map(map)

    map = {1: "Vaginal", 2: "Cesáreo"}
    ...
    map = {0: "Risco: Apgar baixo (<7)", 1: "Apgar saudável (>7)"}

    df["APGAR5"] = df["APGAR5"].map(map)

    map = {1: "Branca", 2: "Negra"}
    ...
    map = {0: "Sem anomalias", 1: "Risco: Presença de anomalias"}
    ...
    df["MESPRENAT"] = df["MESPRENAT"].apply(lambda x: f"Inicio PN: {x}° mês")

    df["CODOCUPMAE"] = df["CODOCUPMAE"].apply(lambda x: f"Ocupação: {x}")
...
def apply_apriori(df: pd.DataFrame):
    ...
    item_sets, rules = apriori(
        main_list, min_support=0.25, min_confidence=0.75, max_length=5
    )
    return rules
